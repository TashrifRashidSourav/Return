...
const PostScreen = () => {
  ...
  const fetchMotivation = async () => {
    ...
    try {
      const response = await fetch(`http://10.10.200.209:5000/api/quotes`, {
        headers: {
  ...
  const fetchPosts = async (page: number) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`http://10.10.200.209:5000/api/posts?page=${page}&limit=10`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const likePost = async (postId: string) => {
    ...
    try {
      const response = await fetch(`http://10.10.200.209:5000/api/posts/like/${postId}`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const deletePost = async (postId: string) => {
    ...
    try {
      const response = await fetch(`http://10.10.200.209:5000/api/posts/delete/${postId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const editPost = async () => {
    ...
    try {
      const response = await fetch(`http://10.10.200.209:5000/api/posts/update/${editingPost._id}`, {
        method: 'PUT',
        headers: {
  ...
  const renderPost = ({ item }: { item: Post }) => (
    <View style={styles.postContainer}>
      ...
      {item.imageUrl && (
        <Image
          source={{ uri: `http://10.10.200.209:5000${item.imageUrl}` }}
