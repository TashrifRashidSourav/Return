...
const PostCreateScreen = () => {
  ...
  const fetchPosts = async () => {
    ...
    try {
      const response = await fetch('http://192.168.0.101:5000/api/posts/my-posts', {
        headers: {
  ...
  const handleCreateOrUpdatePost = async () => {
    ...
    try {
      ...
      const url = editingPost
        ? `http://192.168.0.101:5000/api/posts/update/${editingPost}`
        : 'http://192.168.0.101:5000/api/posts/create';
  ...
  const handleEditPost = (post: any) => {
    setText(post.text);
    setImage(post.imageUrl ? `http://192.168.0.101:5000${post.imageUrl}` : null);
      ...
      {loading ? (
        ...
        posts.map((post) => (
          <View key={post._id} style={styles.post}>
            ...
            {post.imageUrl && <Image source={{ uri: `http://192.168.0.101:5000${post.imageUrl}` }} style={styles.postImage} />}
