...
const RoutineScreen: React.FC = () => {
  ...
  const fetchRoutines = async () => {
    ...
    try {
      const response = await fetch(
        `http://192.168.0.101:5000/routines/${formatDate(date)}`,
        {
  ...
  const addRoutine = async () => {
    ...
    try {
      const response = await fetch('http://192.168.0.101:5000/routines', {
        method: 'POST',
        headers: {
  ...
  const deleteRoutine = async (id: string) => {
    ...
    try {
      const response = await fetch(`http://192.168.0.101:5000/routines/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
